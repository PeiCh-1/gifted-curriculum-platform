...
2. 規劃每週進度：
   - 每週挑選 1~2 個最切合的指標代碼。
   - 【資優調整規範】：**嚴禁整段刪除**！必須以原始指標內容為底稿，僅針對必要處進行擴充或細節調整。**必須且僅限**使用 [+新增內容+] 與 [-刪除內容-] 的格式進行標註，遵循「最小變更原則」。**嚴禁加入任何額外的注記文字（如「新增：」、「刪除：」）**，確保內容可直接提交審核。
   - **單元重點字數限制**：單元重點 (lessonFocus) 只能根據課程內容生成 **10 個字以內** 的重點，不可超過。
   - **同步勾選行政欄位**：請根據每週教學重點，同時挑選 1~2 項最適合的「評量方式」以及必要融入的「議題」。
